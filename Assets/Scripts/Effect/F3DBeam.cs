...
public class F3DBeam : MonoBehaviour
{
    ...
    void Raycast()
    {
        ...
        if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
        {
            ...
            if (rayImpact)
            {
                var shield = hitPoint.transform.gameObject.GetComponent<Forcefield>();
                if (shield != null)
                    shield.OnHit(hitPoint.point);
                rayImpact.position = hitPoint.point;
                var beam = GetComponent<Beam>();
                var ship = hitPoint.transform.gameObject.GetComponentInParent<Ship>();
                if (beam != null && ship !=null)
                    ship.TakeDamage(beam.Damage);
            }
        }
